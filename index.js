...
const mapsAPIKey = "AIzaSyDjzyHAwe9x1NZRVr_s6DFY9nY2okuXIj8";
...
function convertLocToLatLong(location, dist) {
  ...
  let searchURL =
    geocodeURL + `address=${encodeURIComponent(location)}&key=${mapsAPIKey}`;
  ...
  fetch(searchURL)
    ...
    .then(responseJSON => {
      ...
      $(".header")
        .removeClass("h-100 align-items-center")
        .addClass("align-items-start");
      $("#main-search")
        .removeClass("header-before")
        .addClass("header-after");
      $("#slogan")
        .empty()
        .html(`Go on a hike. Treat yo'self with a beer`)
        .addClass("after-results");
    })
    .catch(err => {
    ...
    for (let i = 0; i < responseJSON.trails.length; i++) {
      $(".js-hike-results").append(`
        <li class='ind-hike-result'>
          <h4><a href='${responseJSON.trails[i].url}' target='blank'>${
        responseJSON.trails[i].name
      }</a></h4>
          <p><i>${responseJSON.trails[i].location}</i>
          <br>Length: ${responseJSON.trails[i].length} miles
          <br>${responseJSON.trails[i].stars}/5 stars based on ${
        responseJSON.trails[i].starVotes
      } reviews</p>
        </li>
      `);
...
function getHikeResults(coordinates, dist) {
  ...
  let hikeSearchURL =
    ...
    `lat=${coordinates.lat}&lon=${
      coordinates.lng
    }&key=${hikeAPIKey}&maxDistance=${dist}`;
...
function displayBeerResults(results, status) {
  ...
  if (status == google.maps.places.PlacesServiceStatus.OK) {
    for (let i = 0; i < results.length; i++) {
      $(".js-beer-results").append(`
        <li class='ind-beer-result'>
          ...
          <p>${results[i].rating}/5 stars based on ${
    ...
    $(".js-beer-results").html(
      `<p>but, we couldn't find any breweries! Try increasing your search distance or entering a new location.</p>`
    );
  }
}
